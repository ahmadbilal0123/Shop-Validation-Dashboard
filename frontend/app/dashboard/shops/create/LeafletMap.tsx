...
type LeafletMapProps = {
	...
	onMapClick?: (lat: number, lng: number) => void // NEW
}

export default function LeafletMap({ lat, lng, markers, selectedPinIdx, onMapClick }: LeafletMapProps) {
	...
	useEffect(() => {
		if (selectedPinIdx != null && markerRefs.current[selectedPinIdx] && mapInstanceRef.current) {
			...
			markerRefs.current.forEach((m) => { if (m && m.setZIndexOffset) m.setZIndexOffset(0) })
	...
	if (error) {
		return (
			<div style={{ height: "400px", width: "100%", display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "#f8f9fa", border: "1px solid #dee2e6", borderRadius: "8px", color: "#6c757d" }}>
				{error}
			</div>
		)
	}
