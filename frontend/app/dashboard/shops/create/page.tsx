...
export default function CreateShopPage() {
  ...
  // Get API base URL from env variable (must start with NEXT_PUBLIC_)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Get token from session in localStorage
      ...
      <Card className="w-full max-w-2xl shadow-2xl border-blue-100 bg-white/80 backdrop-blur-xl">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Select Location on Map</label>
              <div className="rounded-lg overflow-hidden border border-blue-200 h-72">
                <LeafletMap
                  lat={gps?.lat || 31.5204}
                  lng={gps?.lng || 74.3587}
                  markers={gps ? [{ lat: gps.lat, lng: gps.lng, label: "Selected Location" }] : []}
              ...
              <div className="mt-2 text-sm text-slate-700">
                {gps
                  ? <>
                      ...
                      <span className="ml-2">Longitude (E)</span>
                      <span className="ml-2">Latitude (N)</span>
                    </>
                  : <span className="italic text-slate-400">No location selected. Click on the map to select.</span>
                }
              </div>
            </div>
            {error && (
            ...
            <div className="pt-4 flex gap-2 justify-end">
              ...
              <Button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white" disabled={!shopName || !shopAddress || !gps || loading}>
